...
/** roles: cargos que pueden ver el link. Vacío = todos. */
  ...
  template: `
    ...
    >
      ...
      <nav class="flex flex-col gap-0.5 px-2 py-3 flex-1">
        @for (link of visibleLinks; track link.href) {
          <a
...
export class SidebarComponent {
  ...
  /** Links filtrados según el cargo del usuario autenticado */
  get visibleLinks(): NavLink[] {
    const cargo = this.auth.cargo();
    return this.allLinks.filter(link =>
      !link.roles || link.roles.includes(cargo)
    );
  }
